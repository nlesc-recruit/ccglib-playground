...
wmma_complex_gemm_basic_interleaved(C_t C, const A_t A, const B_t B) {
  ...
  const unsigned tid = blockDim.x * blockDim.y * threadIdx.z +
  ...
  for (unsigned i = tid; i < M_PER_BLOCK * K_PER_BUFFER * COMPLEX;
       i += block_size) {
  ...
  for (unsigned n = 0; n < N_; n += N_WMMA) {
    for (unsigned m = 0; m < M_; m += M_WMMA) {
      rocwmma::fragment<rocwmma::accumulator, M_WMMA, N_WMMA, K_WMMA, Tout>
          fragC;
      rocwmma::fill_fragment(fragC, static_cast<Tout>(0));
      for (unsigned k = 0; k < K_; k += K_WMMA) {
        rocwmma::fragment<rocwmma::matrix_a, M_WMMA, N_WMMA, K_WMMA, Tin,
                          rocwmma::row_major>
            fragA;
        rocwmma::fragment<rocwmma::matrix_b, M_WMMA, N_WMMA, K_WMMA, Tin,
                          rocwmma::col_major>
            fragB;
      ...
      rocwmma::store_matrix_sync(
          &(*C_)[block_m_start + m][block_n_start * COMPLEX + n], fragC,
  ...
  const size_t tid = threadIdx.x + blockDim.x * threadIdx.y + blockDim.x *
  ...
  rocwmma::fragment<rocwmma::accumulator, M_WMMA, N_WMMA, K_WMMA, Tout>
  fragC[warp_tiles_m][warp_tiles_n]; for (unsigned n = 0; n < warp_tiles_n; n++)
  ...
  for (unsigned k_tile = 0; k_tile < K_GLOBAL / TILE_SIZE_K; k_tile++) {
      ...
      for (unsigned i = 0; i < BLOCK_TILE_SIZE_M * TILE_SIZE_K * COMPLEX; i +=
  block_size) { const unsigned c = i % COMPLEX; const unsigned k = (i / c) %
  TILE_SIZE_K; const unsigned m = (i / (k + c)); A_s[m][k][c] = A[block_start_m
  + m][k_start + k][c];
      }

      for (unsigned i = 0; i < BLOCK_TILE_SIZE_N * TILE_SIZE_K * COMPLEX; i +=
  block_size) { const unsigned c = i % COMPLEX; const unsigned k = (i / c) %
  TILE_SIZE_K; const unsigned n = (i / (k + c)); B_s[n][k][c] = A[block_start_n
  + n][k_start + k][c];
      }

      __syncthreads();

      // run mma per K tile, note that we load 2 values per K due to complex
  axis for (unsigned k = 0; k < TILE_SIZE_K; k+= K_WMMA / COMPLEX) {
          // load MMA fragments
          rocwmma::fragment<rocwmma::matrix_a, M_WMMA, N_WMMA, K_WMMA, Tin,
  rocwmma::row_major> fragA[warp_tiles_m]; rocwmma::fragment<rocwmma::matrix_b,
  M_WMMA, N_WMMA, K_WMMA, Tin, rocwmma::col_major> fragB[warp_tiles_n];

          for (unsigned warp_tile_m = 0; warp_tile_m < warp_tiles_m; warp_tile_m
  ++) { rocwmma::load_matrix_sync(fragA[warp_tile_m], &A_s[warp_tile_m *
  WARP_TILE_SIZE_M][k][0], COMPLEX * TILE_SIZE_K);
          }

          for (unsigned warp_tile_n = 0; warp_tile_n < warp_tiles_n; warp_tile_n
  ++) { rocwmma::load_matrix_sync(fragB[warp_tile_n], &A_s[warp_tile_n *
  WARP_TILE_SIZE_N][k][0], COMPLEX * TILE_SIZE_K);
          }

          // run MMA
          for (unsigned n = 0; n < warp_tiles_n; n++) {
              for (unsigned m = 0; m < warp_tiles_m; m++) {
                  rocwmma::mma_sync(fragC[m][n], fragA[m], fragB[n],
  fragC[m][n]);
              }
          }
      }
      ...
      for (unsigned n = 0; n < warp_tiles_n; n++) {
              for (unsigned m = 0; m < warp_tiles_m; m++) {
                  rocwmma::store_matrix_sync(&C[block_start_m + m *
  WARP_TILE_SIZE_M][block_start_n + n * WARP_TILE_SIZE_N][0], fragC[m][n],
  N_GLOBAL * COMPLEX, rocwmma::mem_row_major);
          }
      }

      // ensure all threads are done before new data is written to shared memory
