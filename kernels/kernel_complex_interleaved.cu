...
extern "C" __global__ void wmma_complex_gemm_basic_interleaved(C_t C, const A_t A, const B_t B) {
    ...
    for (unsigned m = 0; m < TILES_M; m++) {
      for (unsigned n = 0; n < TILES_N; n++) {
        for (unsigned k = 0; k < K_ / K_WMMA; k++) {
          ...
          wmma::load_matrix_sync(fragA, &(*A_)[m * M_WMMA][k * K_WMMA], K_);
          wmma::load_matrix_sync(fragB, &(*B_)[n * N_WMMA][k * K_WMMA], K_);
  ...
  for (unsigned m = 0; m < TILES_M; m++) {
    for (unsigned n = 0; n < TILES_N; n++) {
      wmma::store_matrix_sync(&(*C_)[block_m_start + m * M_WMMA][block_n_start * COMPLEX + n * N_WMMA], fragC[m][n],
                              N_GLOBAL * COMPLEX, wmma::mem_row_major);
    }
  }
}
